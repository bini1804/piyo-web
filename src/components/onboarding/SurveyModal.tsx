...
export default function SurveyModal({
  ...
  const canProceed = () => {
    ...
    if (step === 1) return Boolean(data.skin_type);
    if (step === 2) return selectedConcerns.length > 0;
    return false;
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-[4px]">
      <div
        className="relative flex max-h-[calc(100dvh-2rem)] w-full max-w-lg flex-col overflow-hidden bg-white animate-slide-up shadow-[var(--shadow-md)]"
          ...
          {step === 0 && (
            <div className="animate-fade-in space-y-5">
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={skinDataConsent}
                  onChange={(e) => setSkinDataConsent(e.target.checked)}
                  className="mt-1 w-4 h-4 rounded border-[var(--border)] shrink-0"
